...
% alignlvl : alignement level for activty curve, usually 0.9 (90%) or 0.5 (50%)
...
if E == 2
    ...
    for ni = 1:nc
       ...
       % Autocorrelation
        ...
        if (T(end) - T(1) > 300) && ((T(end)-T(1))/(length(T)-1) <= 1) % done only for curves > 5 min and with consistency in time
            
            
            [C,L] = FFTandAUTOCORR(T,D3,2); % performs cure autocorrelation. 
            % This function also does FFT but here it is desactivated as it is unsused
            
            ptr = L<301; % only taking the first five minutes
