...
%             plot([X2tmp-3 X2tmp-3 X2tmp+3 X2tmp+3 X2tmp-3],[Y2tmp-Leng-2 Y2tmp+Leng+2 Y2tmp+Leng+2 Y2tmp-Leng-2 Y2tmp-Leng-2],'y-*')
            ...
            % Test pour voir si length(ROI1) < 2 * Leng + 5, ie si l'image
            % est trop croppée pour pouvoir prendre la ROI verticale
            % (meilleure à priori).
            % > [rY1tmp-Leng-2 rY1tmp+Leng+2] est de longueur 75 si pas
            % trop croppé.
