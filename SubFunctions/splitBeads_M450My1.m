function [X1,Y1,A1,ptr1,X2,Y2,A2,ptr2,S] = splitBeads_M450My1(X_ind,stackname,filename,Y,Area,distcheck,AUTO,tag)
...
while not(exist('X1','var'))
    ...
    if length(ptr_img) == 2 && max(Area(ptr_img))>300 % deux billes sur l'image
    ...
    elseif length(ptr_img) > 2 && max(Area(ptr_img))>300% plus de deux billes sur l'images
        ...
        if exist('Xim','var')
            ...
            for ii = 1:length(ptr_img)
                ...
                if abs(sqrt((X_ind(ptr_img(ii),1)-Xim(1))^2 + (Y(ptr_img(ii))-Yim(1))^2)) > distcheck ...
                        && abs(sqrt((X_ind(ptr_img(ii),1)-Xim(2))^2 + (Y(ptr_img(ii))-Yim(2))^2)) > distcheck
...
for i=img:nimg
    ...
    if length(ptr_i) == 2 && max(Area(ptr_i))>300% deux bille sur l'image
    ...
    elseif length(ptr_i) > 2 && max(Area(ptr_i))>300%  plus de 2 billes sur l'image
