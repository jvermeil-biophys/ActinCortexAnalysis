...
while not(exist('X1','var'))
    ...
    ptr_img = find(X_ind(:,2) == img); % Renvoie le nombre positions X associées à l'image 'img'
    ...
    elseif length(ptr_img) > 2 % Plus de deux billes sur l'images
        ...
        if ismember(img,Slog) % Est-ce que l'utilisateur a déjà pointé les billes dans un run précédent ?
            ...
            I = imadjust(I); % Ajuster luminosité & contraste
            figure
            imshow(I)
            hold on
            plot(X_ind(ptr_img),Y(ptr_img),'o','color',[1 0.3 0],'markersize',5) % Afficher les centres des billes détectées.
            title(['Click the 2 beads to track. ' stackname ' Slice ' num2str(img) '/' num2str(nimg)])
            % Etape où l'utilisateur sélectionne les billes sur l'image
...
% Q: A QUOI SERT CE TEST ??  A: C'est un cas ou on va rien faire et passer à l'image suivante     
