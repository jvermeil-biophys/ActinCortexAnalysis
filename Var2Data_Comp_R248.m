function Var2Data_Comp_R248(date,tag,Bcorrec,manip,specif,nimgbr,nimgar,...
    nimgr1,nimgr2,nimgr3,Db,resfolder,figurefolder)
...
%       version (_Comp_R248) is used to analyze successive compression
%       of 2, 4, and 8 seconds (only a few experiments were done that way,
...
%
%       Var2Data_Comp_R248(date,tag,Bcorrec,manip,specif,nimgbr,nimgar,...
%           nimgr1,nimgr2,nimgr3,Db,resfolder)
%
...
%       nimgbr : number of images before a ramp
%       nimgar : number of images after a ramp
%       nimgr1 : number of images for 2s ramp
%       nimgr2 : number of images for 4s ramp
%       nimgr3 : number of images for 8s ramp
...
if exist([path filesep loadname],'file')
    ...
    for kc = 1:nc
        ...
        if not(isempty(MT{kc}))
            ...
            CompDurs = []; % list of compression durations
            ...
            NimgtotInit = length(BTMat); % total number of images
            ...
            % counting number of ramps of each duration
            while Nbt>0
                ...
                elseif Nbt == 0
                    n1 = n1 +1 ; % last ramp present is a 2s ramp
                    
                    CompDurs = [CompDurs; '2s'];
                ...
                elseif Nbt >0
                    ...
                    if Nbt < 0                        
                        error('Incoherence with cycle sizes or incomplete cycle')
                    elseif Nbt == 0
                        n2 = n2 +1 ; % last ramp present is a 4s ramp
                        
                    CompDurs = [CompDurs; '4s'];
                    elseif Nbt >0
            ...
            krempl = [nimgbr+nimgar+nimgr1];  % end of loop 1          
            nimgcycle = [nimgbr+nimgar+nimgr3 nimgbr+nimgar+nimgr1 nimgbr+nimgar+nimgr2];            
            for ii = 2:n1+n2+n3 % finding positions of end of loops
                krempl(ii) = krempl(ii-1) + nimgcycle(mod(ii,3)+1);
            end                           
            Tfull2(krempl) = Tfull2(krempl-1)+0.05; % applying correction            
            Bfull2 = [Bfull(2:end); Bfull(end)]; % same for B            
            ...
            RmpDat = cell(1,n1+n2+n3); % variable for storing individual ramps
            CstRmp = cell(1,n1+n2+n3); % variable for constant data relative to ramp
            
            % begining of ramps
            kbeg(1:3:length(krempl)) = krempl(1:3:length(krempl))-nimgr1-nimgar+1;
            kbeg(2:3:length(krempl)) = krempl(2:3:length(krempl))-nimgr2-nimgar+1;
            kbeg(3:3:length(krempl)) = krempl(3:3:length(krempl))-nimgr3-nimgar+1;
            
            % end of ramps
            kend(1:3:length(krempl)) = krempl(1:3:length(krempl))-nimgar;
            kend(2:3:length(krempl)) = krempl(2:3:length(krempl))-nimgar;
            kend(3:3:length(krempl)) = krempl(3:3:length(krempl))-nimgar;
            ...
            for k = 1:n1+n2+n3
                ...
                CompDuration{k} = CompDurs(k,:); % duration of ramp
