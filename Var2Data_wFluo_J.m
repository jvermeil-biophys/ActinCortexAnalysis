...
function NotSaved = Var2Data_wFluo_J(date,chmpMag,manip,specif,ExperimentalConditions,datafolder,resfolder,resfolderbis)
...
MAGFIELDMAGNET = ExperimentalConditions.MagFieldMagnet;
DIAMETER = ExperimentalConditions.DiametreBilles;
...
%                 error(['No bead number available for ' name])
            ...
            L = (L1+L2)/2*SCALE*1000; % distance moyenne parcourue entree deux images en nm
            ...
            if FE
                ...
                B = MAGFIELDCORRECTIONCOEFF*BTMat(S,1)+MAGFIELDMAGNET;
...
%             plot(T,T,'*')
            ...
            mB = find(B < 0.8*MAGFIELDCORRECTIONCOEFF*str2double(chmpMag(1:end-2)));
            ptrdel2 = [ptrdel2 mB'];
            end
            ...
            if min(D3)<0
                Valid = 0;           
                cprintf('err', ' Negative thickness\n');
                pause(0.5)
            end 
            ...
            if ~isempty(ptrdeld3)
               ...
               for kp = 1:length(ptrdeld3)
                  if ptrdeld3(kp)>length(dd3) || (abs(dd3(ptrdeld3(kp)))>200 &&...
...
%                 end
        ...
        %                 xlabel('Time(s)','FontName','Serif')
                    ...
                    ax3 = subplot(5,1,3);
                    hold on
                    plot(T,D2-DIAMETER,'m-')
                    plot(T,D2-DIAMETER,'kx')
        %                 xlabel('Time(s)','FontName','Serif')
                    xticklabels(ax3,{})
