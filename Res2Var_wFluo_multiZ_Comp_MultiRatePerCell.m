...
%       using the depthograph. This particular version
%       (_wFluo_multiZ_Comp_MultiRatePerCell)
%       is used for analyzing compression experiments with different time
%       for each compression. In the experiments the constant
...
%
%       Res2Var_wFluo_multiZ_Comp_MultiRatePerCell(restype,date,tag,manip,
%       specif,fich, depthoname,rmptimetag, AUTO,datafolder,resfolder)
%
...
%       rmptimetag : tag of the txt file generated by labview containing
%       the information on ramp duration and nb of img per loops
    ...
    if E == 2
        if AUTO
            ...
            try
                ...
                [Srmp, X1rmp, X2rmp, Y1rmp, Y2rmp, dzrmp, Scst, X1cst, X2cst, Y1cst, Y2cst, dzcst, Sramp] = doAnalysis(date, manip, Noim,  path, resname, tag, restype, stackname, name, rmptimename, depthoname, datafolder, AUTO);
...
function [Srmp, X1rmp, X2rmp, Y1rmp, Y2rmp, dzrmp, Scst, X1cst, X2cst, Y1cst, Y2cst, dzcst, Sramp] = doAnalysis(date, manip, Noim,  path, resname, tag, restype, stackname, name, rmptimename, depthoname, datafolder, AUTO)
...
rmptimeMat = dlmread([path filesep rmptimename],'\t',0,0);
