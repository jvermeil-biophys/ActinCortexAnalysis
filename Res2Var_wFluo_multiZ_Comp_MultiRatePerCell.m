function Res2Var_wFluo_multiZ_Comp_MultiRatePerCell(restype,date,tag,manip,specif,fich,depthoname,rmptimetag,...
...
for ki=1:nacq
    ...
    rmptimename = [name '_' rmptimetag '.txt'];
    ...
    if E == 2
        ...
        try
            ...
            rmptimeMat = dlmread(rmptimename,'\t',0,0);
            
            nimgbefrmps = rmptimeMat(:,2);
            
            nimgrmps = rmptimeMat(:,3);
            ...
            for k = 1:size(rmptimeMat,1)-1
               
                Sramp = [Sramp sum(nimgloops(1:k-1))+nimgbefrmps(k)+1:sum(nimgloops(1:k-1))+nimgbefrmps(k)+nimgrmps(k)];
               
                
            end
            
             
                Scst = Sfull(~ismember(Sfull,Sramp));
