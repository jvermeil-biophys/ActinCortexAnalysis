function Res2Var_wFluo_multiZ_Comp_J(restype,date,manip,tag,specif,ExperimentalConditions,fich,depthoname,nimgbr,nimgr,nimg,...
...
%         try
            ...
            ResultsMatrix=dlmread(resname,'\t',1,0);
            fid = fopen(resname);
            HeadAll = fgetl(fid);
            fclose(fid);
            ...
            [X1tot,Y1tot,S1tot,ptr1,X2tot,Y2tot,S2tot,ptr2] = splitBeads([ResultsMatrix(:,Nxm) S],...
                [path filesep stackname],[path filesep name],ResultsMatrix(:,Nym),AUTO,restype2);
            ...
            % pour les points en multiZ on récupère X et STD selon down/mid/up
            ...
            X1d(pd1) = X1tot(p1d);
            STD1(1,pd1) = ResultsMatrix(ptr1(p1d),Nstd);
            X2d(pd2) = X2tot(p2d);
            STD2(1,pd2) = ResultsMatrix(ptr2(p2d),Nstd);
            ...
            X1m(pm1) = X1tot(p1m);
            STD1(2,pm1) = ResultsMatrix(ptr1(p1m),Nstd);
            X2m(pm2) = X2tot(p2m);
            STD2(2,pm2) = ResultsMatrix(ptr2(p2m),Nstd);
            ...
            X1u(pu1) = X1tot(p1u);
            STD1(3,pu1) = ResultsMatrix(ptr1(p1u),Nstd);
            X2u(pu2) = X2tot(p2u);
            STD2(3,pu2) = ResultsMatrix(ptr2(p2u),Nstd);
            ...
            STD1rmp = ResultsMatrix(ptr1(p1rmp),Nstd);
            STD2rmp = ResultsMatrix(ptr2(p2rmp),Nstd);
            ...
            dzcst = dzcst*OPTICALDEPTHCORRECTIONCOEFF; 
            dzrmp = dzrmp*OPTICALDEPTHCORRECTIONCOEFF;
