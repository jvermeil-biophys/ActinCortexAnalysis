function outvar = Data2MecaLoop(date,manip,specif,tag,DIAMETER,resfolder,figurefolder,PLOT,VERBOSE)
...
%fieldpath = [datafolder filesep date(7:8) '.' date(4:5) '.' date(1:2)]; % INUTILE, -> datafolder supprimé des variable entrée
...
% Param pour fit
LoopCont = 0;
LoopDef = 1; % conditionné a loopcont = 0
...
if exist([resfolder filesep 'MecaDataTable.mat'])
    ...
    varnames = {'ExpType', 'ExpDay', 'CellID', 'CompNum'   , 'TpsComp', 'RawDataTDFB', 'InitialThickness', 'H0Chadwick', 'EChadwick', 'CiEChadwick', 'FitParams', 'Validated', 'Comments'};
    vartypes = {'string' , 'string', 'string', 'doublenan' , 'string' , 'cell'       , 'doublenan'       , 'doublenan' , 'doublenan', 'doublenan'  , 'string'   ,'logical'  , 'string'};
    BigTable = table('Size',[1 length(varnames)],'VariableTypes',vartypes,'VariableNames',varnames);
end
        ...
        if not(isempty(MR{i}))
            ...
            for ii = 1:nComp
                if not(ischar(RampData{ii}))
                    ...
                    % Check is this comp has already been analyzed
                    CurrentCell = (strcmp(BigTable.ExpType,specif)) & (strcmp(BigTable.ExpDay,date)) & ...
                        (strcmp(BigTable.CellID,currentCellName{i})) & (BigTable.CompNum == ii);
                    ...
                    elseif AlreadyIn == 1
                        ...
                        CurrentTableLine = find(AlreadyIn);
                        
                    else
                        CurrentTableLine = TableSize;
                    ...
                    BigTable(CurrentTableLine,'CellID') = {currentCellName{i}};
                    ...
                    Dup = RampData{ii}(indexThresholdBtot:indexMaxBtot,3) - DIAMETER/1000; % Distance (en µm) pendant la phase de compression
                    Ddown = RampData{ii}(indexMaxBtot:indexThresholdBtotBis,3) - DIAMETER/1000; % Distance (en µm) pendant la phase de décompression
                    ...
                    while abs((H0prev(end)-H0tmp)/H0prev(end)) > 0.01
                        ...
                        try
                            ...
                            notsavedtext = ['Fit error : ' fitError.message];
                            canBeFitted = 0;
                            break
                        end
                ...
                if canBeFitted
                    ...
                    if SAVING_CRITERION
                        ...
                        StrainRateMed{i} = [StrainRateMed{i}; median(ExpStrainRateInterp)];
                    ...
                    if PLOT
                        ...
                        if SAVING_CRITERION
                            ...
                            text(mean(RampData{ii}(:,2)),max(RampData{ii}(:,3)*1000-DIAMETER),...
                                [num2str([isPositive hasNormalDz hasSmallConfInt hasNoLargeJump hasEnoughPoints doesConverge isAGoodFit])...
                                ' posi/Dz/CI/Jump/points/converge/goodfit'],...
                    ...
                    CompressionFeatures(firstCompressionIndex + ii).hasEnoughPoints = hasEnoughPoints;
                    ...
                    CompressionFeatures(firstCompressionIndex + ii).CouldBeFitted = 'Yes';
                    
                else
                    CompressionFeatures(firstCompressionIndex + ii).CouldBeFitted = ['No, sorry...'];
                    CompressionFeatures(firstCompressionIndex + ii).WhatHappend = notsavedtext;
                ...
                elseif canBeFitted
                    ...
                    BigTable(CurrentTableLine,'Validated') = {0};
                    BigTable(CurrentTableLine,'Comments') = {'Couldn''t be fitted'};
                end
                %%% [/data saving] %%%
...
save([sf filesep 'D2M_' date '_' manip '_' specif '.mat'],'Echad','EchadConf','H0abs','ChadsH0','CompNum','StrainRateMed','CompressionFeatures','params')
