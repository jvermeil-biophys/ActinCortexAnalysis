function Var2Data_Comp_MultiRatePerCell(date,tag,Bcorrec,manip,specif,rmptimetag,...
    Db,datafolder,resfolder,figurefolder)
...
%       version (_Comp_MultiRatePerCell) is used to analyze compressions
%       experiments with different cmpression length over one video.
%       This Var2Data also removes some points from the curves but doesn't
%       remove whole curves like Var2Data_wFluo.
%
%       Var2Data_Comp(date,tag,Bcorrec,manip,specif,rmptimetag,...
%       ,Db,datafolder,resfolder,figurefolder)
%
...
%       datafolder : path to raw data with the txt file for ramp durations
...
txtpath = [datafolder filesep date(7:8) '.' date(4:5) '.' date(1:2)]; % where to find text file for comp durations
...
if exist([path filesep loadname],'file')
    ...
    for kc = 1:nc
        ...
        if not(isempty(MT{kc}))
            VALID = 1;
            name = MT{kc}.exp;
            rmptimedataname = [name '_' rmptimetag '.txt']; % name of text file
            ...
            rmptimeMat = dlmread([txtpath filesep rmptimedataname],'\t',0,0);
            ...
            Tfull2(cyclesend) = Tfull2(cyclesend-1)+0.05;         
            ...
            nimgbefrmps = rmptimeMat(:,2); % images before each ramp
            
            nimgloops = rmptimeMat(:,4); % images for each loops
            
            cyclesends = [0; cumsum(nimgloops)]; % end of loops
            
            
            for k = 1:ncycle
                
                CompDur = num2str(rmptimeMat(k,1)/2); % retrieving comp duration
                ...
                CompDur = [CompDur 's']; % comp duration in correct format
                
                % getting data for ramp n°k
